...
def _deepmd_env() -> str:
    """Return the DeepMD conda environment root directory from platform.yaml."""
    py = load_platform_config().get("hpc", {}).get("python_deepmd", "")
    return str(Path(py).parent.parent) if py else ""

def _account() -> str:
    """Return the gpu_h100 Slurm account from platform.yaml."""
